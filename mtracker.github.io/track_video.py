...
parser.add_argument("-v", "--path_to_video", dest='v', help="path to the folder containing the demo video",
                    default="./demo/demo_video.mp4")
parser.add_argument("-t", "--model_type", dest="t",
                    help="specify which type of model to use (u for unet and du for dense unet)", default="du")
parser.add_argument("-m", "--model", dest="m",
                    help="path to the model", default="./model/dense_compound.hdf5")
parser.add_argument("-o", "--output_folder", dest="o",
                    help="path to the output folder", default="./demo/output.csv")
parser.add_argument("-f", "--figure", dest="f",
                    help="path to save figures", default="./demo/")
parser.add_argument("-b", "--boundary", dest="b", nargs=4,
                    help="the boundary for cropping the central portion of images", default=[106, 385, 121, 487])
parser.add_argument("-n", "--n_frames", dest="n",
                    help="plot every Nth frame (default 100)", default=100)
...
if args.t == "du":
    model = DenseNet.DenseUnet_v2(weights=None, input_shape=[
                                  128, 128, 3], loss="compound")
...
with open(path_to_csv_output, mode='w') as out:
    out = csv.writer(out, delimiter=',', quotechar='"',
                     quoting=csv.QUOTE_MINIMAL)
    out.writerow(['x', 'y', 'uniqueframe'])
    for index, image in tqdm(enumerate(video)):
        try:
